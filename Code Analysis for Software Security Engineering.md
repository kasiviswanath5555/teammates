...
+ For the assurance claim “Login Module of TeamMates is acceptably secure to brute force attack”.
  ...
  + #### Automated code Scanning 
    + Automated code Scanning through Find bugs highlighted some security vulnerabilities i.e. Potential CRLF injection for logs which can impact the log activities. TeamMates enforces secure input data validation to prevent the exploitation of logs. Also the automated code scanning refers to the use of Cipher c = Cipher.getInstance ("AES/ECB/PKCS5Padding"); in the StringHelper class(which are used for encrypting the userid's) as being insecure but TeamMates already enforces secure practices like using HMAC-MD5 algorithm which is first applied on the data(cryptoHelper class)and then the cryptographic functions are applied.
